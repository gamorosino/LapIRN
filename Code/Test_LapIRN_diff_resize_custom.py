...
def test():
    ...
    fixed_img, header, affine = load_4D(fixed_path, get_info=True)
    moving_img, header, affine = load_4D(moving_path, get_info=True)
    
    ori_imgshape = fixed_img.shape[1:]
    ...
    with torch.no_grad():
        ...
        F_X_Y_xyz_cpu = F_X_Y_xyz.cpu().numpy()[0]

        F_X_Y_xyz_cpu = F_X_Y_xyz_cpu.transpose(1, 2, 3, 0)
        ...
        save_img(F_X_Y_xyz_cpu, f"{savepath}/Warp.nii.gz", header=header, affine=affine)
        #F_X_Y_xyz_cpu_ = -1 * F_X_Y_xyz_cpu
        #save_img(F_X_Y_xyz_cpu_, savepath+"/Warp_neg.nii.gz", header=header, affine=affine)
